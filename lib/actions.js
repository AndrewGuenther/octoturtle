...
const applyLabels = function applyLabels(labels) {
  return (event, context, github) => {
    github.applyLabels(context, labels);
  };
};
...
const addComment = function addComment(comment) {
  return (event, context, github) => {
    github.addComment(context, comment);
  };
};
 ...
 */
const postStatus = function postStatus(state, targetUrl, description, statusContext) {
  return (event, context, github) => {
    github.postStatus(context, state, targetUrl, description, statusContext);
  };
};
