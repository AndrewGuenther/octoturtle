...
class Github {
   ...
   * @param {Payload} payload A webhook payload
   */
  constructor(user, token, payload) {
   ...
   */
  applyLabels(labels) {
    return this.sendRequest(this.payload.getLabelUrl(), labels);
  }
   ...
   */
  addComment(comment) {
    return this.sendRequest(this.payload.getCommentUrl(), comment);
  }
  ...
  postStatus(state, targetUrl, description, context) {
    return this.sendRequest(this.payload.getStatusUrl(), {
      state,
