...
class Github {
   ...
   */
  constructor(user, token, context) {
   ...
   */
  applyLabels(labels) {
    return this.sendRequest(this.context.getLabelUrl(), labels);
  }
   ...
   */
  addComment(comment) {
    return this.sendRequest(this.context.getCommentUrl(), comment);
  }
  ...
  postStatus(state, targetUrl, description, context) {
    return this.sendRequest(this.context.getStatusUrl(), {
      state,
