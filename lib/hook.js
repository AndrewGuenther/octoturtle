...
/**
 * A set of conditions and response actions for Github webhook contexts.
 */
class Hook {
   ...
   */
  constructor(event, githubUser, githubToken) {
  ...
  /**
   * Applies a filter based on the "action" field of a webhook context.
   *
   ...
   * @param  {Context} context The raw webhook context.
   */
  ...
  /**
   * Given a context, evaluate all specified filters and perform any given
   * reactions.
   *
   * @param  {String} event The webhook event type.
   * @param  {Object} context The raw webhook context.
   * @return {Boolean} Returns true if <em>any</em> action is performed.
   *    Otherwise, returns false.
   */
  eval(event, rawContext) {
    const context = Context.extendContext(rawContext);
    const github = new Github(context, this.githubUser, this.githubToken);
    if ((event !== this.event) ||
        (this.actions.length &&
            this.actions.indexOf(context.getAction()) === -1) ||
        (this.repositories.length &&
            this.repositories.indexOf(context.getRepo()) === -1) ||
        (this.senders.length &&
            this.senders.indexOf(context.getSender()) === -1)) {
      return false;
    }

    let reacted = false;
    this.reactions.forEach((reaction) => {
      reacted = reaction(event, context, github) || reacted;
    });
